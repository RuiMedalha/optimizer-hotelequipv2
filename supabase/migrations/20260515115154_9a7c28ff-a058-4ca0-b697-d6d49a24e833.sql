DROP FUNCTION IF EXISTS public.get_products_page(uuid, text, text, text, text, text, text, text, text, integer, integer, text);
...
BEGIN
  ...
  WITH filtered AS (
    ...
    WHERE p.workspace_id = _workspace_id
      ...
      AND (_product_type = 'all' OR p.product_type::text = _product_type)
      ...
      AND (
        ...
        (p.publishability_decision::text = _publishability_decision)
      )
      AND (
      ...
      AND (
        ...
        (_image_status = 'any_issue' AND (p.image_status IN ('failed','missing') OR p.image_urls IS NULL OR array_length(p.image_urls, 1) = 0))
      )
      AND (
        _woo_filter = 'all' OR 
