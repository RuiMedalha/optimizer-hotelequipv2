...
function buildProductData(p: Record<string, unknown>, onlyMapped: boolean, mappedFieldKeys: Set<string>, hasMapping: boolean, skuPrefix?: string, skuSuffix?: string, modelSuffix?: string, defaultBrand?: string, autoModelFromSku?: boolean) {
  ...
  const fieldMap: Record<string, () => void> = {
    ...
    original_price: () => { data.original_price = parsePrice(p.original_price); },
    ...
    image_alt_texts: () => { data.image_alt_texts = p.image_alt_texts ? String(p.image_alt_texts).split(/[,|]/).map((s: string) => s.trim()).filter(Boolean) : null; },
    stock: () => { data.stock = p.stock !== undefined ? parseInt(String(p.stock), 10) || 0 : null; },
    disponibilidade: () => { data.stock = p.disponibilidade !== undefined ? (String(p.disponibilidade).toLowerCase().includes("em stock") || String(p.disponibilidade).toLowerCase().includes("in stock") ? 999 : parseInt(String(p.disponibilidade), 10) || 0) : null; },
    attributes: () => { if (p.attributes) data.attributes = p.attributes; },
    weight: () => { data.weight = p.weight ? parseFloat(String(p.weight).replace(",", ".")) || null : null; },
    dimensions: () => { data.dimensions = toStr(p.dimensions, 200); },
    model: () => { data.model = modeloVal; },
    video_url: () => { data.video_url = toStr(p.video_url, 500); },
    certifications: () => { data.certifications = p.certifications ? String(p.certifications).split(/[,|]/).map((s: string) => s.trim()).filter(Boolean) : null; },
    seo_slug: () => { data.seo_slug = toStr(p.seo_slug, 200); },
    tags: () => { data.tags = p.tags ? String(p.tags).split(/[,|]/).map((s: string) => s.trim()).filter(Boolean) : null; },
    is_discontinued: () => { data.is_discontinued = p.is_discontinued === "true" || p.is_discontinued === true || String(p.is_discontinued).toLowerCase() === "sim"; },
    min_order_quantity: () => { data.min_order_quantity = p.min_order_quantity ? parseInt(String(p.min_order_quantity), 10) || 1 : 1; },
